...
# Obtener el ID del dispositivo (excluyendo el dispositivo de arranque "aa888d1f")
device=$(adb devices | grep -v 'aa888d1f' | grep -v 'List' | awk '{ print $1 }')

# Intentar obtener las versiones de Z-Wave y PowerG
zwave_version=$(adb -s $device shell "getprop | grep zwave.frequency")
powerg_version=$(adb -s $device shell "getprop | grep powergv4.fw")

# Esperar hasta obtener ambos valores, o hasta agotar el número máximo de intentos
cont=0
max_tries=3
